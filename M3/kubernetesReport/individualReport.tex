...
When implemented, Network Policies control traffic by filtering traffic and implementing network segregation. This allows the different components to be isolated and have a controlled access to resources. This minimizes the attack surface and it plays a crucial role in maintaining integrity and confidentiality of applications.

By default, Kubernetes clusters have no policies set, which can cause security issues since all pods are allowed to communicate freely with any other system. The administrator must explicity configure Network Policies to ensure a controlled and restrict environment within the cluster \cite{amaechi_learn_2023}.

Before creating any Network Policies, the cluster must use a network plugin that supports the Network Policy enforcement.
...
As previously mentioned, no Network Policies are set by default. This means that all outbound and inbound traffic is allowed. When this behavior is restricted by explicitly defining the rules, only connections specified in the Egress section of a NetworkPolicy that applies to the pod are allowed. Additionally, reply traffic for those allowed connections is implicitly permitted. \cite{walker_kubernetes_2023}
...
An example from \cite{network_policies_kubernetes_from_kubernetes_io} is used to demonstrate how to define Network Policies:
...
In this picture \ref{code:netPolicies}, the Network Policy defined that:

\begin{enumerate}
	\item This policy applied to all pods with role equal to \texttt{db} inside the \texttt{default} namespace
	\item In this policy, both Ingress and Egress rules are set
...
From the example provided in \ref{code:netPolicies} the following main components can be identified:
