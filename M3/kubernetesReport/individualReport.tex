...
\publishers
{
	...
	\begin{versionhistory}
		\vhEntry{1}{2024-10-28}{Pedro Moreira}{Initial version and structure}  
		\vhEntry{2}{2024-11-10}{Pedro Moreira}{Added Background}
		\vhEntry{3}{2024-11-18}{Pedro Lemos}{Added Network Policies}
		\vhEntry{4}{2024-11-19}{Luis Correia}{Added Data Encryption}
		\vhEntry{5}{2024-11-26}{Pedro Moreira}{Added RBAC and Authorization}
		\vhEntry{6}{2024-11-26}{Pedro Lemos}{Added Complementary Tooling}
		\vhEntry{7}{2024-12-04}{Luis Correia}{Added API Server Configurations}
		\vhEntry{8}{2024-12-16}{Luis Correia Pedro Lemos Pedro Moreira}{Added Analysis and Best Practices}
		\vhEntry{9}{2024-12-20}{Luis Correia Pedro Lemos Pedro Moreira}{Added Conclusions}    
...
Now that the \texttt{Policy} manifest has been created, it must be referenced in the API configuration so that it can be used by the cluster. This is done by using the \texttt{audit-policy-file} and \texttt{audit-log-path} fields when configuring the API, as shown in Figure \ref{fig:auditconf}. Bear in mind that, for this example, we're outputting the logs to the standard output, which is the logs of the \texttt{kube-apiserver} pod. In a production scenario, there might be other optimal approaches. One such approach is the usage of a webhook backend through the \texttt{--audit-webhook-config-file} configuration, which will send the "audit events to a remote web API" \cite{the_linux_foundation_auditing_2024}.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{figs/audit-namespace-create.png}
...
Use a default \texttt{deny-all} network policy, an example of which is presented in \ref{code:denyAllPolicy}. This ensures that only explicitly defined resources can communicate with the Pod.
